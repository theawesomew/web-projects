define([
	...
	var PreLoader = Class.extend({
		...
		load: function(files, _path, _path2) {
			...
			for (var i = 0, len = files.length; i < len; i++) {
				...
				switch (path.split(".").pop()) {
					...
					case "mp3":
					case "wav":
						var _loop = file.length > 2 ? file[2] : false;
						this.loadSound(name, path, _loop, null);
						break;
		...
		loadSound: function(name, path, loop, callback) {
			this.pending++;
			loop = loop || false;
			var sound = this.files[name] = new Sound(path, loop);
