define([
	...
	var Camera = Class.extend({
		...
		setBounds: function() {
			...
			if (this.width > this.bounds.width) {
				this.bounds.x += (this.bounds.width - this.width)/2;
				this.bounds.width = this.width;
			}
			if (this.height > this.bounds.height) {
				this.bounds.y += (this.bounds.height - this.height)/2;
				this.bounds.height = this.height;
			}

			this.bounds.width  += this.bounds.x - this.width;
			this.bounds.height += this.bounds.y - this.height;
		},
		getBounds: function() {
			return this.bounds;
		},
		...
		isWithin: function(ent) {
			var p = ent.getPos();
			var r = new Rect(this.position.x, this.position.y, this.width, this.height);

			return r.within(p.x, p.y);
		},
